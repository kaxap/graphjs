...
(function (root, factory) {
  ...
  // (Mathematica Journal, 2005). The headline feature is the ADAPTIVE
  // STEP SIZE: the algorithm tracks the total system energy each
  // iteration and grows the step when the layout is improving, shrinks
  // it when energy goes up. This converges much faster than FR's linear
  // cooling — and adapts naturally to different graph scales.
  //
  // Force model:
  //   Repulsion: f_rep = C · K² / d         (weaker than FR by factor C)
  //   Attraction: f_att = d² / K            (same as FR)
  //
  // The C ≈ 0.2 weakens repulsion so attraction dominates at typical
  // edge distances, producing tighter clusters than FR.
  //
  // This implementation is O(N²) per iteration like FA2/FR. Hu's
  // original paper pairs this with Barnes-Hut quadtree approximation
  // for O(N log N) — a worthwhile extension for >1000-node graphs.

  function yifanHuLayout(nodes, edges, opts) {
    var N = nodes.length;
    if (N === 0) return { positions: {}, width: 0, height: 0 };

    var iterations  = opts.iterations  != null ? opts.iterations  : 300;
    var nodeWidth   = opts.nodeWidth  || 200;
    var nodeHeight  = opts.nodeHeight || 60;
    // K = natural spring length (equilibrium edge distance).
    var K           = opts.optimalDistance != null ? opts.optimalDistance
                    : opts.K != null ? opts.K : 100;
    // C = relative strength of repulsion (~0.2 in Hu's paper).
    var C           = opts.repulsion != null ? opts.repulsion : 0.2;
    // Step-size cooling ratio (Hu uses 0.9). Smaller = more aggressive.
    var stepRatio   = opts.stepRatio != null ? opts.stepRatio : 0.9;
    // Initial step size. Defaults to K (one edge length per iteration).
    var initialStep = opts.initialStep != null ? opts.initialStep : K;
    // Early-exit when per-node energy drops below this.
    var tolerance   = opts.tolerance != null ? opts.tolerance : 1e-4;
    // Gravity toward origin, like in FA2/FR.
    var gravity     = opts.gravity != null ? opts.gravity : 0.1;
    var preventOverlap = opts.preventOverlap === true;
    var seed        = opts.seed != null ? opts.seed : 1;
    ...
    var nEdges = edges.length;
    var eS = new Int32Array(nEdges);
    var eT = new Int32Array(nEdges);
    var validEdges = 0;
    for (var e = 0; e < nEdges; e++) {
      var ed = edges[e];
      var si = idx[ed.source], ti = idx[ed.target];
      if (si == null || ti == null || si === ti) continue;
      eS[validEdges] = si; eT[validEdges] = ti;
      validEdges++;
    }

    var step = initialStep;
    var energy = Infinity;
    var progress = 0; // consecutive successful (energy-decreasing) iterations
    var K2 = K * K;
    var CK2 = C * K2;
    ...
    for (var iter = 0; iter < iterations; iter++) {
      ...
      // Update positions, accumulate energy (sum of squared force magnitudes).
      for (var u = 0; u < N; u++) {
        var fm2 = fx[u] * fx[u] + fy[u] * fy[u];
        energy += fm2;
        var fm = Math.sqrt(fm2);
        if (fm > 0) {
          x[u] += step * fx[u] / fm;
          y[u] += step * fy[u] / fm;
        }
      ...
      // Adaptive step size — the Yifan Hu trick. After 5 consecutive
      // energy-decreasing iterations, grow the step; whenever energy
      // increases, shrink it immediately.
      if (energy < prevEnergy) {
        ...
        progress = 0;
        step *= stepRatio;   // shrink
      }

      // Early exit on convergence.
      if (energy < tolerance * N * K2) break;
    }
  ...
  // ---------- OpenOrd layout -------------------------------------------
  //
  // Martin, Brown, Boyack & Klavans, "OpenOrd: An Open-Source Toolbox for
  // Large Graph Layout" (2011). Designed for very large graphs (millions
  // of nodes) where FR/FA2 are too slow or get stuck in local minima.
  // Its signature: cleanly separated cluster "blobs" with whitespace
  // between them.
  //
  // The trick is a 5-stage temperature/attraction schedule:
  //   1. liquid    — extreme temperature, weak attraction; random mixing
  //   2. expansion — high temperature, weak attraction; nodes spread out
  //   3. cooldown  — moderate temperature, normal attraction; structure emerges
  //   4. crunch    — low temperature, strong attraction; clusters tighten
  //   5. simmer    — minimal temperature, strongest attraction; final settling
  //
  // Early stages also do RANDOM JUMPS — occasional teleports that help
  // nodes escape local minima before settling. Real OpenOrd also does
  // sub-quadratic repulsion via random sampling and a density grid;
  // this implementation keeps the staged schedule and random jumps but
  // sticks with O(N²) repulsion for consistency with FA2/FR/YH.

  function openOrdLayout(nodes, edges, opts) {
    var N = nodes.length;
    if (N === 0) return { positions: {}, width: 0, height: 0 };

    var iterations = opts.iterations != null ? opts.iterations : 500;
    var nodeWidth  = opts.nodeWidth  || 200;
    var nodeHeight = opts.nodeHeight || 60;
    var K          = opts.K != null ? opts.K : 100;
    var gravity    = opts.gravity != null ? opts.gravity : 0.05;
    var preventOverlap = opts.preventOverlap === true;
    var seed       = opts.seed != null ? opts.seed : 1;
    // Per-stage parameters can be overridden in opts.stages.
    // Defaults follow the OpenOrd paper's recommended schedule.
    var stages = opts.stages || [
      { name: "liquid",    frac: 0.25, tempK: 2.0,  attract: 0.2, jump: 0.05  },
      { name: "expansion", frac: 0.25, tempK: 1.5,  attract: 0.5, jump: 0.02  },
      { name: "cooldown",  frac: 0.25, tempK: 0.6,  attract: 1.0, jump: 0.005 },
    ...
    for (var s = 0; s < stages.length; s++) {
      var stage = stages[s];
      var stageIters = Math.max(1, Math.floor(stage.frac * iterations));
      var temperature = stage.tempK * K;
      var attractFactor = stage.attract;
      var jumpProb = stage.jump || 0;

      for (var local = 0; local < stageIters; local++) {
        for (var z = 0; z < N; z++) { fx[z] = 0; fy[z] = 0; }
        ...
        // Attraction scaled by stage: f = attractFactor · d² / K
        for (var ei = 0; ei < validEdges; ei++) {
          var sIdx = eS[ei], tIdx = eT[ei];
          var ax = x[sIdx] - x[tIdx];
          var ay = y[sIdx] - y[tIdx];
          var ad2 = ax * ax + ay * ay;
          if (ad2 < 0.01) ad2 = 0.01;
          var ad = Math.sqrt(ad2);
          var af = attractFactor * ad2 / K;
          var afx = (ax / ad) * af, afy = (ay / ad) * af;
          fx[sIdx] -= afx; fy[sIdx] -= afy;
          fx[tIdx] += afx; fy[tIdx] += afy;
        }

        // Gravity toward origin (small constant).
        if (gravity > 0) {
          for (var gi = 0; gi < N; gi++) {
            var gd = Math.sqrt(x[gi] * x[gi] + y[gi] * y[gi]);
            if (gd < 0.01) gd = 0.01;
        ...
        // Apply displacement, capped by stage temperature, with optional
        // random jumps (the "escape from local minima" trick that
        // distinguishes OpenOrd's liquid + expansion stages).
        for (var u = 0; u < N; u++) {
          var disp = Math.sqrt(fx[u] * fx[u] + fy[u] * fy[u]);
          if (disp > 0) {
            var capped = Math.min(disp, temperature);
            x[u] += (fx[u] / disp) * capped;
            y[u] += (fy[u] / disp) * capped;
          }
          if (jumpProb > 0 && rand() < jumpProb) {
            x[u] += (rand() - 0.5) * temperature;
            y[u] += (rand() - 0.5) * temperature;
          }
        }
      }
    }
  ...
  var LAYOUTS = {
    ...
    "yifan-hu": yifanHuLayout,
    yifanHu:    yifanHuLayout,
    yh: yifanHuLayout,
    openord:  openOrdLayout,
    openOrd:  openOrdLayout,
    oo:       openOrdLayout,
  };
